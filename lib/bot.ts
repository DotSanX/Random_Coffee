...
import { MyContext, Database, Tables } from "./interfaces.ts"; //импорт интерфейсов
...
bot.command("start", async (ctx) => { // бот получает команду /start
  const userId = Number(ctx.msg.from?.id);
  const info = (await users.select().eq("id", userId)).data
  await ctx.reply(`${info}!`);
  // if ((await users.select().eq("id", userId)).status == 200) {
  //   const info = (await users.select().eq("id", userId)).data
  //   await ctx.reply(`Привет, ${info}!`, { reply_markup: menuKeyboard });
  // } else {
  //   await ctx.reply(
