import { Bot, Context, InlineKeyboard, Keyboard } from "https://deno.land/x/grammy@v1.32.0/mod.ts";
...
// info нужна для созранения инфо пользователя в бд (или получения)
const info: UserInfo = {
  name: "",
  age: 0,
  interests : [],
...
bot.callbackQuery("interestsNotDone", async ctx=>{
  await ctx.deleteMessage()
  await ctx.answerCallbackQuery("Хорошо, напиши еще увлечений!")
  state = "setInterests";
})
  ...
  if (state) { // при непустом state
    switch (state) {
      ...
      case "setGeo":
        ...
        await ctx.reply(
          "А теперь расскажи мне немного о себе. Перечисли через запятую свои интересы или выбери уже заготовленные варианты! 🙌🏻",
        );
        state = "setInterests";
        break;
